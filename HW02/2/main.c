...
int check_file( char *filename )
{
  ...
  if( (unsigned long int)file_size < sizeof( LFH ) ||
      (unsigned long int)file_size < sizeof( CDFH )
    )
  {
  ...
  ptr_up = buffer + ( (long)file_size - (long)sizeof( LFH ));
...
//  if( (uint32_t)P->Signature == (uint32_t)PK_LFH_SIGNATURE )
      ...
      )
    {
      ...
      if(zipout!=NULL)
      {
        fwrite( ptr, 1, file_size - (long)(ptr - buffer), zipout );
        fflush( zipout );
        fclose( zipout );
      }
  ...
  ptr_up = buffer + ( (long)file_size - (long)sizeof( CDFH ));
